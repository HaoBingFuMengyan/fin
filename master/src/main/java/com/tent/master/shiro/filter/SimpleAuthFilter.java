package com.tent.master.shiro.filter;

import com.tent.common.utils.Lg;
import com.tent.master.shiro.session.CustomSessionManager;
import com.tent.master.shiro.session.SessionStatus;
...
public class SimpleAuthFilter extends AccessControlFilter {
	...
	protected boolean isAccessAllowed(ServletRequest request,
		...
		if (null != sessionStatus && !sessionStatus.isOnlineStatus()) {
			...
			if (ShiroFilterUtils.isAjax(request) ) {
				Lg.info(SimpleAuthFilter.class, "当前用户已经被踢出，并且是Ajax请求！");
	...
	protected boolean onAccessDenied(ServletRequest request,
		 ...
		 * 传统的解决方法是变成/hy/login.shtml?redirectUrl=/admin/index.html。
